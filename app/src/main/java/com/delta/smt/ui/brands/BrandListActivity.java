...
public class BrandListActivity extends BaseActiviy<BrandPresenter> implements BrandContract.View, SuperRecyclerView.LoadingListener {
    ...
    protected void initView() {
        commonBaseAdapter = new CommonBaseAdapter<Brands.DataBean.ItemsBean>(getContext(), datas) {
            ...
            protected void convert(CommonViewHolder holder, Brands.DataBean.ItemsBean item, int position) {
                ...
                Glide.with(getContext()).load(item.getData().getCover_image_url()).placeholder(R.mipmap.ic_loading_rotate).error(R.mipmap.ic_logo).into(((ImageView) holder.getView(R.id.iv)));
            }
